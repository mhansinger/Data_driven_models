...
def compose_db(case,train_test_set,scaler):
    ...
    data_pq = dd.read_parquet(join(path_to_data,'filter_width_*_GAUSS_DNN_'+case+'_'+train_test_set+'.parquet'),chunksize='2GB')
    ...
    if train_test_set == 'train':
        if scaler == 'Log':
            ...
            moments.to_csv(join(path_to_data, 'moments_' + case + '_GAUSS_Log.csv'))
        elif scaler == 'Standard':
            print('\nComputing moments for Standard Scaler...')
            moments.to_csv(join(path_to_data, 'moments_' + case + '_GAUSS.csv'))
    ...
    for i in tqdm(range(1,files+1),desc='Computing ...',ncols=75):
        ...
        ## PARQUET
        if scaler=='Log':
            data_df.to_parquet(filename + '_GAUSS_Log.parquet')
        elif scaler=='Standard':
            data_df.to_parquet(filename + '_GAUSS.parquet')
